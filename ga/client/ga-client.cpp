...
pthread_mutex_t count_lock = PTHREAD_MUTEX_INITIALIZER;  //互斥量
pthread_cond_t count_nonzero = PTHREAD_COND_INITIALIZER;  //条件变量
...
void* decrement_count(void* arg)
{
	...
	while (countThread == 0)
	{
		printf("decrement_count countThread:%d\n", countThread);
		pthread_cond_wait(&count_nonzero, &count_lock);
	}
	countThread = countThread - 1;
	pthread_mutex_unlock(&count_lock);
	printf("decrement_count end\n");
	return NULL;
}
...
void* increment_count(void* arg)
{
	...
	countThread = 2;
	// pthread_mutex_unlock(&count_lock);
	printf("increment_count countThread:%d\n", countThread);
	pthread_cond_signal(&count_nonzero);
	printf("increment_count countThread:%d\n", countThread);
	pthread_mutex_unlock(&count_lock);
	printf("increment_count end\n");
	return NULL;
}
...
int main1(int argc, char *argv[]){
	...
	pthread_cond_signal(&count_nonzero);
	Sleep(3000);
	pthread_create(&thid1, NULL, decrement_count, NULL);
	Sleep(5000);
	pthread_create(&thid2, NULL, increment_count, NULL);
	while (true)
	{
		Sleep(1000);
	}
...
{
	sem_wait(&m_SemReady);
	printf("thread:%lu is running %lu \n", pthread_self(), pthread_self());
	int rv = 44;
...
{
	//sem_wait(&m_SemReady);
	printf("thread_func1:%lu is running %lu \n", pthread_self(), pthread_self());
	void *ignored;
	pthread_join(thid, &ignored);
...
int main3(int argc, char *argv[]){	
	...
	pthread_create(&thid, NULL, thread_func, NULL);
	pthread_t thid1;
	pthread_create(&thid1, NULL, thread_func1, NULL);

	Sleep(3000);
	if (sem_trywait(&m_SemReady) == -1)
	{
		printf("main is -2\n");
		sem_post(&m_SemReady);
	}
