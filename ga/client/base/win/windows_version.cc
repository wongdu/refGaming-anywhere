...
static inline RTL_OSVERSIONINFOW winOsVersion()
{
	...
	//hNtdll = GetModuleHandle(u"ntdll.dll");
	//hNtdll = GetModuleHandle(std::wstring("ntdll.dll").c_str());
    if (hNtdll) {
...
#pragma warning(pop)
        ...
        RTL_OSVERSIONINFOW osVersionInfo = winOsVersion();
        version_info.dwMajorVersion = osVersionInfo.dwMajorVersion;
        version_info.dwMinorVersion = osVersionInfo.dwMinorVersion;
        version_info.dwBuildNumber = osVersionInfo.dwBuildNumber;
