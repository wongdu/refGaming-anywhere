		...
		for (int i = 0; i < rtspconf->vso->size(); i += 2) {
			if ((*rtspconf->vso)[i].compare("b") == 0) {
				if (reconf->bitrateKbps > 0)
					(*rtspconf->vso)[i + 1] = std::to_string(static_cast<unsigned long long>(reconf->bitrateKbps * 1000));
				continue;
			}
			if ((*rtspconf->vso)[i].compare("crf") == 0) {
				if (reconf->crf > 0)
					(*rtspconf->vso)[i + 1] = std::to_string(static_cast<unsigned long long>(reconf->crf));
				continue;
			}
		}
...
#ifdef SAVE_ENC
			if (fout != NULL) {
				fwrite((&pkt)->data, sizeof(char), (&pkt)->size, fout);
			}
#endif
			// send the packet
...
#define	MAXPARAMLEN	64
	...
	if(vencoder_started != 0)
		return 0;
	vencoder_started = 1;
	ga_log("vencoder_x264_ start_thread:%lu is running %lu \n", pthread_self(), pthread_self());
	for(iid = 0; iid < video_source_channels(); iid++) {
...
vencoder_stop(void *arg) {
	...
	if(vencoder_started == 0)
		return 0;
	vencoder_started = 0;
	ga_log("vencoder_x264_ stop_thread:%lu is running %lu \n", pthread_self(), pthread_self());
	for(iid = 0; iid < video_source_channels(); iid++) {
