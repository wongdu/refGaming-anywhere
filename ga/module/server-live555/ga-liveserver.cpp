...
static const int subtypeRtcpApp = 10;
	...
	for (mi = sinkmap.begin(); mi != sinkmap.end(); mi++) {
		...
		while ((stats = statsIter.next()) != NULL) {
			...
			if ((strcmp("H264", mi->first->rtpPayloadFormatName()) == 0 ||
				...
				pSubsession->sendRTCPAppPacket(subtypeRtcpApp, "videoRTD", (unsigned char*)arrFeedbackRtd, strlen(arrFeedbackRtd));
