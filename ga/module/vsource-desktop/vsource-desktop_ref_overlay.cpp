...
static HBITMAP hBmp;
static BITMAP bmp;
static unsigned char* lpBitmapData;

static CImg<unsigned char> cimgHouse;
int overlay_bmp_cimg(vsource_frame_t *frame);
int overlay_bmp(vsource_frame_t *frame);
...
static void * vsource_threadproc(void *arg) {
	...
	hBmp = (HBITMAP)LoadImage(NULL, ".\\videoWnd_bg.bmp", IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION | LR_LOADFROMFILE);
	GetObject(hBmp, sizeof(bmp), &bmp);
	unsigned int bitmapDataSize = bmp.bmHeight*bmp.bmWidth*bmp.bmBitsPixel;
	bitmapDataSize >>= 3;
	lpBitmapData = (unsigned char*)malloc(bitmapDataSize);
	memset(lpBitmapData, 0, bitmapDataSize);
	GetBitmapBits(hBmp, bitmapDataSize, lpBitmapData);

	/*cimgHouse.load(".\\house.jpg");*/
	cimgHouse.load(".\\sh.jpg");
...
int overlay_bmp_cimg(vsource_frame_t *frame){
	int  ptx, pty;
	unsigned char* fcurr;
	for (int i = 0; i < cimgHouse._height; i++) {
		pty = 20 + i;
		if (pty < 0)
			continue;
		...
		for (int j = 0; j < cimgHouse._width; j+=3) {
		//for (int j = 0; j < cimgHouse._width; j ++) {
			ptx = 20 + j/3*4;
			if (ptx < 0)
				continue;
			...
			/*fcurr[ptx * 4 + 0] = cimgHouse._data[i*cimgHouse._width + j * 4 + 0];
			fcurr[ptx * 4 + 1] = cimgHouse._data[i*cimgHouse._width + j * 4 + 1];
			fcurr[ptx * 4 + 2] = cimgHouse._data[i*cimgHouse._width + j * 4 + 2];*/

			/*fcurr[ptx * 3 + 0] = cimgHouse._data[i*cimgHouse._width + j * 3 + 0];
			fcurr[ptx * 3 + 1] = cimgHouse._data[i*cimgHouse._width + j * 3 + 1];
			fcurr[ptx * 3 + 2] = cimgHouse._data[i*cimgHouse._width + j * 3 + 2];*/

			/*fcurr[ptx * 3 + 0] = cimgHouse._data[i*cimgHouse._width + j  + 0];
			fcurr[ptx * 3 + 1] = cimgHouse._data[i*cimgHouse._width + j  + 1];
			fcurr[ptx * 3 + 2] = cimgHouse._data[i*cimgHouse._width + j  + 2];*/

			fcurr[ptx + 0] = cimgHouse._data[i*cimgHouse._width + j + 0];
			fcurr[ptx + 1] = cimgHouse._data[i*cimgHouse._width + j + 1];
			fcurr[ptx + 2] = cimgHouse._data[i*cimgHouse._width + j + 2];
			//ptx++;

	/*		fcurr[ptx + 0] = cimgHouse._data[i*cimgHouse._width + j + 2];
			fcurr[ptx + 1] = cimgHouse._data[i*cimgHouse._width + j + 1];
			fcurr[ptx + 2] = cimgHouse._data[i*cimgHouse._width + j + 0];*/

			/*fcurr[ptx * 3 + 0] = cimgHouse._data[i*cimgHouse._width + j * 3 + 2];
			fcurr[ptx * 3 + 1] = cimgHouse._data[i*cimgHouse._width + j * 3 + 1];
			fcurr[ptx * 3 + 2] = cimgHouse._data[i*cimgHouse._width + j * 3 + 0];*/
		}
	}
	return 0;
}
...
int overlay_bmp(vsource_frame_t *frame){
	...
	if (!lpBitmapData){
		ga_error("get the overlay content failed!\n");
		return -1;
	}
	int  ptx, pty;
	unsigned char* fcurr;
	...
	for (int i = 0; i < bmp.bmHeight; i++) {
		...
		if (pty >= frame->realheight)
			break;
		ccurr = lpBitmapData + i * bmp.bmWidthBytes;
		fcurr = frame->imgbuf + (pty * frame->realstride);
